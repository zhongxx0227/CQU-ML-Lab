...
Date: 2021-11-22 16:56:44
LastEditors: XiangxinZhong
LastEditTime: 2021-11-23 00:02:16
Description: ML Lab3 决策树回归
'''
...
class Reg(object):
    ...
    def fit(self, data, labels, max_depth=5, min_samples_split=2):
...
'''
description: 预测结果可视化，icecream 二维
param {*} x 横坐标
param {*} y_pred 预测值
param {*} y_true 真实值
return {*}
...
def drawResult(x, y_pred, y_true):
    plt.figure(dpi=200)
    # 真实值散点图
    plt.scatter(x, y_true, c='b', cmap='viridis',
                edgecolors='black', label='true')
    plt.scatter(x, y_pred, c='r', cmap='viridis',
                edgecolors='black', label='prediction')
    # 预测值
    # plt.plot(x, y_pred, color='cornflowerblue', label='prediction',linewidth=0.2)
    plt.xlabel('data')
    plt.ylabel('target')
...
if __name__ == "__main__":
    ...
    print("Timestamp {:.2f}, 开始训练".format(time_start))
    reg.fit(X_train, y_train, max_depth=5)
    time_end = time.time()
    time_span = time_end-time_start
    print("Timestamp {:.2f}, 结束训练".format(time_end))
    y_pred = reg.predAll(X_test)
    mse = mtr.mean_squared_error(y_test, y_pred)
    print("经过了{:.2f} s的训练，均方误差为: {}".format(time_span, mse))
